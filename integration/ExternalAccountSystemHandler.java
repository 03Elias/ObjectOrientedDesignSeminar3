package integration;

import model.dto.SaleDTO;

public class ExternalAccountSystemHandler{

    /**
     * A constructor that creates/starts an instance of the External Account System Handler that communicates
     * with the External Account System. 
     */
    public ExternalAccountSystemHandler(){

    }

    /**
     * updates the external account system after a sale has occured.  
     * @param saleInfo The sale information that contains the total price, costs, etc that is needed to update the account. 
     */
    public void updateExternalAccountSystem(SaleDTO saleInfo){

    }
}